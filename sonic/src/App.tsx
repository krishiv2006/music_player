...
import { fetchTrending, searchTracks, fetchByGenre, fetchRecommendations, fetchLyrics, preloadTrack } from './api';
...
export default function App() {
  ...
  const startCrossfade = useCallback(() => {
    ...
    nx.src = `http://localhost:3001/api/stream?q=${encodeURIComponent(nextTrack.title + ' ' + nextTrack.artist)}&duration=${secs}&_t=${Date.now()}`;
    nx.volume = 0;
    nx.play().catch(() => {});
  ...
  const playTrack = useCallback((track: Track, newQueue?: Track[]) => {
    ...
    const src = `http://localhost:3001/api/stream?q=${encodeURIComponent(track.title + ' ' + track.artist)}&duration=${secs}&_t=${Date.now()}`;
